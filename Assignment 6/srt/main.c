...
scene_t scene;
enum { nColors = 3 };
float color[width][height][nColors];

void * thread_render_image (void *ptr)
{
    Vec3 camera_pos;
...
main( int argc, char **argv )
{
    ...
    int thrNumHolder[nthreads];
    int i;
    int j;
    for (i = 0; i < nthreads; i++)
    {
        thrNumHolder[i] = i;
        int rtn1 = pthread_create(&threads[i], NULL, thread_render_image, (void*) &thrNumHolder[i]);
        if (rtn1 != 0)
        {
    ...
    for (i = 0; i < nthreads; i++)
    {
        int rtn2 = pthread_join(threads[i], NULL);
        if (rtn2 != 0)
        {
